 ...
 * @author Ariel Arnedo
 */
public class PanelViewColaEnCaja extends javax.swing.JPanel implements java.util.Observer{
    
    private final Control control;
    

    public PanelViewColaEnCaja(Control control) {
    ...
    public void update(Observable o, Object arg) {
        if(arg instanceof String && String.valueOf(arg).compareTo("UPDATE_VIEW_QUEUE") == 0){
            if(this.getComponents().length != 0)
                this.removeAll();
            control.getColaDeCaja().stream().forEach((persona) -> {
                JLabel icon;
